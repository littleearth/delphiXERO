...
type
  ...
  TXmlOptions = set of (doNodeAutoIndent, doCompact, doParseProcessingInstr,
    doPreserveWhiteSpace, doCaseInsensitive, doWriteBOM);
...
{$ENDIF}
  ...
  protected
    [Weak]
    FDocument: TXMLParser;
    procedure SetDocument(Value: TXMLParser);
  ...
  public
    ...
    // Tristan Marlow
    function SetTextFromDateTime(ADateTime: TDateTime): TXmlNode;
    function SetTextFromDate(ADate: TDate): TXmlNode;
    function SetTextFromTime(ATime: TTime): TXmlNode;
    function AsDateTime: TDateTime;
    function AsDate: TDate;
    function AsTime: TTime;
    procedure SetAttributeFromDateTime(const AttrName: String;
      const ADateTime: TDateTime);
    procedure SetAttributeFromTime(const AttrName: String; const ATime: TTime);
    procedure SetAttributeFromDate(const AttrName: String; const ADate: TDate);
    function GetAttributeAsDateTime(const AttrName: String): TDateTime;
    function GetAttributeAsTime(const AttrName: String): TTime;
    function GetAttributeAsDate(const AttrName: String): TDate;
    function GetChildNodeText(AName: string): string;
    function GetChildNodeAsDateTime(AName: string): TDateTime;
    function GetChildNodeAsDate(AName: string): TDate;
    function GetChildNodeAsTime(AName: string): TTime;
    // Tristan Marlow
    ...
    property Document: TXMLParser read FDocument write SetDocument;
  ...
  public
    ...
    function HasNode(const Name: String; NodeTypes: TXmlNodeTypes = [ntElement])
      : Boolean; virtual;
  ...
  protected
    ...
    function ExtractText(var Line: String; const StopChars: String;
  ...
  public
    ...
    function LoadFromFile(const FileName: String; BufferSize: Integer = 4096)
      : TXMLParser; virtual;
    /// <summary> Loads the XML from a stream </summary>
    function LoadFromStream(const Stream: TStream; BufferSize: Integer = 4096)
      : TXMLParser; virtual;
    /// <summary> Parse attributes into the attribute list for a given string </summary>
    procedure ParseAttributes(const AttribStr: String;
      AttributeList: TXmlAttributeList); virtual;
    /// <summary> Saves the XML to a file </summary>
    function SaveToFile(const FileName: String): TXMLParser; virtual;
    /// <summary> Saves the XML to a stream, the encoding is specified in the .Encoding property </summary>
    function SaveToStream(const Stream: TStream): TXMLParser; virtual;
...
function TXMLParser.AddChild(const Name: String;
...
procedure TXMLParser.CreateHeaderNode;
begin
...
function TXMLParser.CreateNode(const Name: String; NodeType: TXmlNodeType)
  : TXmlNode;
begin
...
function TXMLParser.GetChildNodes: TXmlNodeList;
begin
  Result := Root.ChildNodes;
end;

function TXMLParser.GetEncoding: String;
begin
  if assigned(FHeader) then
...
function TXMLParser.GetPreserveWhitespace: Boolean;
begin
  Result := doPreserveWhiteSpace in Options;
end;

function TXMLParser.GetStandAlone: String;
begin
  if assigned(FHeader) then
...
function TXMLParser.GetVersion: String;
begin
  if assigned(FHeader) then
...
function TXMLParser.IsSame(const Value1, Value2: String): Boolean;
begin
...
function TXMLParser.GetText: String;
var
...
procedure TXMLParser.Compose(Writer: TStreamWriter);
var
  Child: TXmlNode;
begin
...
function TXMLParser.LoadFromFile(const FileName: String;
...
procedure TXMLParser.ParseAttributes(const AttribStr: String;
  AttributeList: TXmlAttributeList);
var
...
begin
  ...
  begin
    ...
    begin
      Quote := Value[LowStr];
      Delete(Value, 1, 1);
      AttrText := ExtractText(Value, Quote, [etoDeleteStopChar]);
      // Get Attribute Value
...
procedure TXMLParser.ParseText(const Line: String; Parent: TXmlNode);
var
  SingleChar: Char;
  Node: TXmlNode;
  TextNode: Boolean;
...
function TXMLParser.ParseTag(const TagStr: String; var Parent: TXmlNode)
  : TXmlNode;
var
  Node: TXmlNode;
  ALine: String;
...
function TXMLParser.SaveToFile(const FileName: String): TXMLParser;
var
  Stream: TFileStream;
begin
...
function TXMLParser.SaveToStream(const Stream: TStream): TXMLParser;
var
...
procedure TXMLParser.SetDocumentElement(Value: TXmlNode);
begin
...
procedure TXMLParser.SetEncoding(const Value: String);
begin
  CreateHeaderNode;
  FHeader.Attributes['encoding'] := Value;
end;

procedure TXMLParser.SetPreserveWhitespace(Value: Boolean);
begin
  if Value then
...
procedure TXMLParser.SetStandAlone(const Value: String);
begin
  CreateHeaderNode;
  FHeader.Attributes['standalone'] := Value;
end;

procedure TXMLParser.SetVersion(const Value: String);
begin
  CreateHeaderNode;
  FHeader.Attributes['version'] := Value;
end;

class function TXMLParser.Unescape(const Value: String): String;
begin
...
procedure TXMLParser.Walk(Writer: TStreamWriter; const PrefixNode: String;
  Node: TXmlNode);
var
  Child: TXmlNode;
  Line: String;
    ...
    ntProcessingInstr:
      begin
        if Node.AttributeList.Count > 0 then
          Writer.Write(Line + '?' + Node.Name +
            Node.AttributeList.AsString + '?>')
        else
          Writer.Write(Line + '?' + Node.Text + '?>');
        Exit;
      end;
...
class function TXMLParser.Escape(const Value: String): String;
begin
  Result := TXmlAttribute.Escape(Value);
  Result := ReplaceStr(Result, '''', '&apos;');
end;

function TXMLParser.ExtractText(var Line: String; const StopChars: String;
...
procedure TXmlNode.SetDocument(Value: TXMLParser);
begin
  FDocument := Value;
...
procedure TXmlNode.SetAttributeFromDateTime(const AttrName: String;
  const ADateTime: TDateTime);
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    SetAttribute(AttrName, Iso8601.DateTimeToIso8601(ADateTime));
  finally
    FreeAndNil(Iso8601);
  end;
end;

procedure TXmlNode.SetAttributeFromTime(const AttrName: String;
  const ATime: TTime);
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    SetAttribute(AttrName, Iso8601.DateTimeToIso8601(ATime));
  finally
    FreeAndNil(Iso8601);
  end;
end;

procedure TXmlNode.SetAttributeFromDate(const AttrName: String;
  const ADate: TDate);
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    SetAttribute(AttrName, Iso8601.DateTimeToIso8601(ADate));
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.GetAttributeAsDateTime(const AttrName: String): TDateTime;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := Iso8601.DateTimeFromIso8601(GetAttr(AttrName));
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.GetAttributeAsTime(const AttrName: String): TTime;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := Iso8601.TimeFromIso8601(GetAttr(AttrName));
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.GetAttributeAsDate(const AttrName: String): TDate;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := Iso8601.DateFromIso8601(GetAttr(AttrName));
  finally
    FreeAndNil(Iso8601);
  end;
end;
...
begin
  Iso8601 := TIso8601.Create;
  try
    Result := SetText(Iso8601.DateToIso8601(ADate));
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.SetTextFromTime(ATime: TTime): TXmlNode;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := SetText(Iso8601.TimeToIso8601(ATime));
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.SetTextFromDateTime(ADateTime: TDateTime): TXmlNode;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := SetText(Iso8601.DateTimeToIso8601(ADateTime));
  finally
    FreeAndNil(Iso8601);
  end;
end;
...
begin
  Iso8601 := TIso8601.Create;
  try
    Result := Iso8601.DateTimeFromIso8601(Text);
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.AsDate: TDate;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := Iso8601.DateFromIso8601(Text);
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.AsTime: TTime;
var
  Iso8601: TIso8601;
begin
  Iso8601 := TIso8601.Create;
  try
    Result := Iso8601.TimeFromIso8601(Text);
  finally
    FreeAndNil(Iso8601);
  end;
end;

function TXmlNode.GetChildNodeText(AName: string): string;
var
  Node: TXmlNode;
begin
  Result := '';
...
function TXmlNode.GetChildNodeAsDateTime(AName: string): TDateTime;
var
  Node: TXmlNode;
begin
  Result := 0;
  Node := Find(AName);
  if Node <> nil then
    Result := Node.AsDateTime;
end;

function TXmlNode.GetChildNodeAsDate(AName: string): TDate;
var
  Node: TXmlNode;
begin
  Result := 0;
...
function TXmlNode.GetChildNodeAsTime(AName: string): TTime;
var
  Node: TXmlNode;
begin
  Result := 0;
...
function TXmlNodeList.HasNode(const Name: String;
  NodeTypes: TXmlNodeTypes = [ntElement]): Boolean;
begin
